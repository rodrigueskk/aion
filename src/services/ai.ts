...
export async function generateTitle(prompt: string): Promise<string> {
  try {
    ...
    const response = await aiClient.models.generateContent({
      model: "gemini-2.0-flash", // Versão estável e ultra rápida para títulos
...
export async function generateResponse(prompt: string, imageBase64?: string, mimeType?: string, modelType: 'thinking' | 'fast' | 'search' | 'as' = 'thinking', customInstruction?: string) {
  try {
    ...
    // MODELO PRINCIPAL (THINKING): O melhor para raciocínio complexo e tarefas escolares
    ...
    if (modelType === 'fast') {
      modelName = "gemini-2.0-flash"; // O mais rápido da atualidade
    ...
    } else if (modelType === 'search' || modelType === 'as') {
      modelName = "gemini-2.0-pro-exp"; // O modelo mais "inteligente" para resumos e correções
      if (modelType === 'search') {
    ...
    if (error.message?.includes("model not found") || error.message?.includes("404")) {
      return `'${modelType}' NÃO ENCONTRADO. Isto é um problema interno que estamos tentando resolver, não se preocupe.`;
    }
