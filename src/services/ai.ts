...
export async function generateTitle(prompt: string): Promise<string> {
  try {
    ...
    const response = await model.generateContent(`Crie um título muito curto (máximo 4 palavras) para: "${prompt}". Retorne apenas o título.`);
    return response.response.text()?.trim() || "Nova Conversa";
  } catch (error: any) {
...
export async function generateResponse(prompt: string, imageBase64?: string, mimeType?: string, modelType: 'thinking' | 'fast' | 'search' | 'as' = 'thinking', customInstruction?: string) {
  try {
    ...
    // DEFINIÇÃO DOS MELHORES MODELOS ATUAIS
    let modelName = "gemini-2.0-flash"; // Padrão ultra rápido e inteligente
    ...
    // INICIALIZAÇÃO DO MODELO COM SYSTEM INSTRUCTIONS (FORMA CORRETA DO SDK)
